...
std::vector<std::vector<int>> multiply(const std::vector<std::vector<int>> &first,
...
std::vector<std::vector<int>> exponentiation(const std::vector<std::vector<int>> &first, const BigInteger &n, int mod) {
  ...
  std::vector<std::vector<int>> current_binary = first;
  for (char i : binary) {
    if (i == '1') {
...
int main() {
  ...
  BigInteger n;
  std::cin >> n >> m >> mod;
  int number_of_masks = 1 << m;
  std::vector<std::vector<int>> ok(number_of_masks, std::vector<int>(number_of_masks));
  //матрица correct
  for (int mask = 0; mask < number_of_masks; ++mask) {
    for (int mask1 = 0; mask1 < number_of_masks; ++mask1) {
      ok[mask][mask1] = correct(mask, mask1, m);
    }
  }
  ok = exponentiation(ok, n - 1, mod);
  long long ans = 0;
  for (int mask = 0; mask < number_of_masks; mask++) {
    for (int mask1 = 0; mask1 < number_of_masks; mask1++)
    ans += ok[mask][mask1];
